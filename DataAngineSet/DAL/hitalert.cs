...
namespace DataAngineSet.DAL
{
    ...
    {
        private readonly DataAngineSet.DAL.hitrecord hitrecorddal = new DataAngineSet.DAL.hitrecord();

        private readonly DataAngineSet.DAL.hitrecord_detail hitrecord_detaildal = new DataAngineSet.DAL.hitrecord_detail();
        ...
        public bool Update(DataAngineSet.Model.hitalert model)
        {
            ...
            for (int n = 0; n < model.details.Length; n++)
            {
                List<DataAngineSet.Model.hitrecord_detail> hitDetails = hitrecord_detaildal.GetModelByHitUserId(model.details[n].user_id);

                foreach (DataAngineSet.Model.hitrecord_detail hitDetail in hitDetails)
                {
                    DataAngineSet.Model.hitrecord hitRecord = hitrecorddal.GetModel(hitDetail.hit_record_id);
        ...
        /// 获得数据列表
        /// </summary>
        public DataSet GetList(string strWhere, string libraryid)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * ");
            strSql.Append(" FROM hitalert ");
        ...
        /// 分页获得数据列表
        /// </summary>
        public DataSet GetList(string strWhere, int startIndex, int pageSize, string libraryid)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * ");
            strSql.Append(" FROM hitalert ");
        ...
        /// 分页获得数据列表
        /// </summary>
        public DataSet GetList_TaskId(string strWhere, string taskid)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * ");
            strSql.Append(" FROM hitalert ");
        ...
        public Model.hitalert DataTableToModel(DataTable table)
        {
            ...
            if (table != null)
            {
                ...
                {
                    model.hit.threshold = decimal.Parse(table.Rows[0]["threshold"].ToString());
                }
                ...
                for (int n = 0; n < table.Rows.Count; n++)
                {
                    ...
                    {
                        detail.score = decimal.Parse(table.Rows[n]["score"].ToString());
                    }
