...
namespace NjustSkyEyeSystem
{
    ...
    class SettingFRS
    {
        ...
        public string[] VideoAddresses
        {
            get { return videoAddresses; }
            set { videoAddresses = value; }
        }
        public SettingFRS()
        {
            ...
            try
            {
                videoAddresses =ConfigurationManager.AppSettings["VideoAddresses"].Split(';');
            }
            catch
            {
                videoAddresses = new string[1] { "" };
            }
        ...
        public static int Save(SettingFRS setting)
        {
            try
            {
                ...
                string addresses = "";
                for (int i = 0; i < setting.VideoAddresses.Length - 1; i++)
                {
                    addresses += (setting.VideoAddresses[i] + ";");
                }
                addresses += setting.VideoAddresses[setting.VideoAddresses.Length - 1];
                config.AppSettings.Settings["VideoAddresses"].Value = addresses;
