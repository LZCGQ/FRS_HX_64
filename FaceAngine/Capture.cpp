...
Capture::Capture(short channelID)
{
#if USE_EXPIRE
	if (IsExpired()) return;
#endif
...
Int32 Capture::Start(Int32 deviceId)
{
	...
	if (nullptr != startThread&&startThread->IsAlive){
		return ReturnCode::ALREADY_RUN;
	}

	startThread = gcnew Thread(gcnew ParameterizedThreadStart(this, &Capture::Begin));

	startThread->Start(deviceId);
	int count = 10;
...
Int32 Capture::Start(String ^streamAddress)
{
	...
	startThread->Start(streamAddress);
...
Int32 Capture::Stop()
{
	...
	int count = 10;
	while (count > 0 && isRun == true){ //等待1秒钟
		Thread::Sleep(100);
		count--;
	}
